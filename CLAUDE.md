...
ルートごとの閲覧許可ロールは `app/utils/permissions.ts` の `ROUTE_PERMISSIONS` が単一の定義（`/team` / `/accounting` / `/profit-loss` / `/recurring-costs` / `/extra-entries` / `/budget-declarations` / `/dashboard`）。`/`, `/matters` はロール制限なし（ログイン必須）。権限クラス×ページの手動確認表は `docs/testing.md` の「3.7 手動確認（RLS・権限クラス別）」を参照。
...
- Supabase 到達不能時の再試行ループで Edge の 25 秒制限に掛からないよう、Auth への 1 リクエストは 5 秒（`AUTH_FETCH_TIMEOUT_MS`）、`getUser()` 全体は 10 秒（`AUTH_GET_USER_TIMEOUT_MS`）で打ち切り、どちらも 503 に落とす（実装は `app/utils/routeGuard.ts` の `createTimeoutFetch` / `withAuthTimeout`）。
