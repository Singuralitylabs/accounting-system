...
| 6   | 損益計算書の計上月判定・月次集計・損益調整・取得期間・月次収支確定・確定後の変更検知 ✅実装済（`tests/utils/profitLossLogic.test.ts`・`tests/utils/profitLossPeriod.test.ts`・`tests/utils/profitLossClosing.test.ts`・`tests/utils/profitLossDiff.test.ts`） | `app/utils/profitLossLogic.ts`・`app/utils/profitLossClosing.ts`・`app/utils/profitLossDiff.ts`                                                                   | `monthDiff` / `isRecurringCostChargedInMonth`（月払い・四半期・年払いの計上月、年度跨ぎ、`end_month` null）/ `buildMonthlyReport`（分類別粗利、費目別管理費、経常利益と刷新前の営業損益の一致、ロール別集計、全体共通費用の分離、損益調整あり／なしの実績額計算、元データ変更検知（`sourceChanged`）、対象行が当月に存在しない調整（`orphanedAdjustments`）の抽出）/ `reportRangeBounds`・`isDateInRangeOrUndated`・`doesRecurringCostOverlapRange`・`isAdjustmentInRange`（期間絞り込みの境界と NULL 行維持、絞り込み前後の集計一致） | 非常に高 | 済（`app/utils/profitLossLogic.ts` へ抽出 + export 済）                                                               | 2        |
