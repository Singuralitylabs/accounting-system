...
2. **純粋関数が module-private**: `profitLossReport.ts` の `monthDiff` / `isRecurringCostChargedInMonth` / `buildMonthlyReport`、`middleware.ts` の `matchesRoute` は export されておらず、かつ前者は `"use server"` ファイル内にあるため直接 import してテストできない。
...
- **観点**
  ...
  - 損益計算書の集計（支払サイクル（月払い/四半期/年払い）の計上月判定、ロール別の集計範囲、チーム未指定（全体共通）費用の扱い）
...
1. **損益計算書の純粋ヘルパー抽出**（#6 の前提）: `monthDiff` / `isRecurringCostChargedInMonth` / `buildMonthlyReport` 等を `"use server"` の付かないファイル（例: `app/utils/profitLossLogic.ts`）へ移動して export する。
2. **`matchesRoute` の共有化**（#7 の前提）: `middleware.ts` 内のプライベート関数を export するか、`app/utils/permissions.ts` へ移動する（`ROUTE_PERMISSIONS` と同居させると凝集度が上がる）。
3. **案件編集ロジックの UI 副作用分離**（#5, #8 の前提）: `editMatterInfo` / `checkMatterInfoList` のバリデーション・遷移判定を純粋関数化し、`confirm` / `alert` / 通知は呼び出し側へ移す。
