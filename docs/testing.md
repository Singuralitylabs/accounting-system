...
| `/profit-loss`                   | **拒否**                               | **表示かつ空でない**（自チーム範囲）。定期費用マスタ・経理追加収支のボタン、「前月の経理追加収支をコピー」ボタンは出ない | **表示かつ空でない**（全体）。定期費用マスタ・経理追加収支のボタンが出る。月次タブに「前月の経理追加収支をコピー」ボタンが出る（前月分がある月では有効、無い月では無効） | **表示かつ空でない**（全体）。同左                                           | 月を選んで売上一覧が数字として出ること。0 円ばかりで内訳が空なら RLS を疑う。ボタンの表示可否がロールと一致すること。コピー実行後に画面へ即時反映されること、teamleader でボタンが出ないことに加え RLS 側でも書き込みが拒否されること（開発者ツールで直接 INSERT を試す等）                                                                                                                                                                                                                                                    |
