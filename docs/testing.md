...
| 22  | 事前収支申告の集計・対象月・表示チーム判定 ✅実装済（`tests/utils/budgetDeclaration.test.ts`）                                                 | `app/utils/budgetDeclaration.ts`                                                                            | `summarizeBudgetItems` の種別別集計と差引 / `addMonths`・`defaultTargetMonth` の年跨ぎと JST 月境界 / `visibleBudgetTeams` のロール別絞り込み（チーム未設定のチームリーダーは対象なし）/ `buildBudgetDeclarationStatusList` の未申告行とマスタ外チームの取りこぼし防止                          | 高       | なし（純粋関数・export 済）                                                                                           | 6        |
