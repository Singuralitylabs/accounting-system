...
| `/profit-loss`         | **拒否**                               | **表示かつ空でない**（自チーム範囲）。定期費用マスタ・経理追加収支のボタンは出ない | **表示かつ空でない**（全体）。定期費用マスタ・経理追加収支のボタンが出る | **表示かつ空でない**（全体）。同左                       | 月を選んで売上一覧が数字として出ること。0 円ばかりで内訳が空なら RLS を疑う。ボタンの表示可否がロールと一致すること                                                              |
...
| `/budget-declarations` | **拒否**                               | **表示かつ空でない**。自チームのみ 1 行                                            | **表示かつ空でない**。全チーム                                           | **表示かつ空でない**。全チーム                           | teamleader に別チームの行が出たら失敗。未申告チームの行が消えていたらチームマスタ側の絞り込みかクエリの不具合。金額が全て「-」なら申告データが無いのか RLS かを切り分ける        |
...
未ログインで保護ページ（`/`, `/matters`, `/matters/team`, `/matters/accounting`, `/new`, `/team`, `/accounting`, `/profit-loss`, `/recurring-costs`, `/extra-entries`, `/budget-declarations`, `/dashboard`）を開くと `/login` へ飛ぶこと。
