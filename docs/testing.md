...
| 21  | 取得エラーの伝播 ✅実装済（`tests/components/dynamicDataErrors.test.tsx` / `AuthProvider.test.tsx`）         | `app/components/dynamic/*` / `app/components/auth/AuthProvider.tsx` / `app/utils/supabase/profiles.ts`      | 取得失敗を「0 件」として描画せず throw する（`initialData` に空配列を渡さない）/ 空の選択肢マスタを `optionsAtom` に投入しない / 予期せぬエラーを再 throw する / `Auth session missing` は children のみ描画                         | 高       | なし（データ取得関数・子コンポーネントはモック）                                                                      | 6        |
