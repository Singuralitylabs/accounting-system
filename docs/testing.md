...
| 23  | PostgREST `PGRST303`（JWT issued at future）の再試行 ✅実装済（`tests/utils/supabase/postgrestFetch.test.ts`）                                 | `app/utils/supabase/postgrestFetch.ts`                                                                      | `/rest/v1/` の PGRST303 だけ同一トークンで再送する / Auth や PGRST301 は再試行しない / refresh しない / `iat` が未来ならその差分を待ち上限で切る                                                                                                                                                                                                                                            | 高       | なし（fetch / sleep を注入）                                                                                          | 6        |
