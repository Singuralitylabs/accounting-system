...
- 損益レポートの絞り込み前後で月次・年間推移の集計値が変わらないこと。特に `invoice_date` / `period` / `entry_date` が NULL の「月未確定」行が脱落していないこと（月未確定の注記と `undated` の値で突き合わせる）
