...
| 17  | 経理一覧の確認完了・フィルタバー ✅実装済（`tests/components/AccountingMatterList.test.tsx` ほか）   | `app/components/matterList/AccountingMatterList.tsx` / `ActiveMatterFilterBar.tsx` / `matterListFilters.ts` | 未選択時の通知 / 表示中リストの `is_fixed` で対象解決 / アクティブフィルタのチップ表示と解除                                                                                                                                      | 高       | Testing Library + jsdom                                                                                               | 6        |
