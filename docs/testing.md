...
- **観点**
  ...
  - 損益計算書の集計（支払サイクル（月払い/四半期/年払い）の計上月判定、案件開始日基準の計上月と下書き除外、案件別収支（チーム → 案件 → 案件内訳）・分類別収支・費目別管理費・経常利益の算出と合計の一致、ロール別の集計範囲、チーム未指定（全体共通）費用の扱い、表示タイトルの解決）
...
- 案件別収支が チーム（項目管理の並び順）→ 案件（ID 昇順）→ 案件内訳（売上 → 費用）で展開でき、チーム小計・合計・分類別収支の合計・サマリーカードが一致すること。品目別の集計行が出ないこと
...
| 確定済みの月の表示                                               | 対象外         | 自チーム＋全体共通（参考）の確定値                                   | 確定値（案件・定期費用を変更・削除しても変わらない）。年間推移の列見出しに鍵アイコン                                  |
