...
| 23  | 事前収支申告の削除フックのキャッシュ無効化 ✅実装済（`tests/hooks/useBudgetDeclarationData.test.tsx`）                                                         | `app/hooks/useBudgetDeclarationData.ts`（`useDeleteBudgetDeclaration`）                                                                                           | 削除エラー時も detail・list を無効化する（Issue #104 の回帰。先行削除の 0 行エラーでも表示が残らない）/ 成功時は detail 破棄 + list 無効化                                                                                                                                                                                                                                                                                                                                                                                             | 高       | Testing Library + jsdom（実 `QueryClient`。DB ヘルパ・通知はモック）                                                  | ―        |
