...
| 18  | Google ログインボタン `SignIn` ✅実装済（`tests/components/auth-components.test.tsx`）               | `app/components/auth/auth-components.tsx`                                                                   | アイコン付きラベル / クリック後の loading（disabled + `aria-busy`）/ OAuth エラー時の再有効化                                                                                                                                     | 中       | Testing Library + jsdom（Supabase / router はモック）                                                                 | 6        |
