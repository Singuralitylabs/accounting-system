...
1. ~~**ブラウザ API との密結合**~~: `alert` は Phase 6-a、`window.confirm` は Phase 6-b で排除済み。確認は `confirmAction()`（`@mantine/modals`）。現行の更新 UI は `updateMatter` を直接呼ぶため、抽出したバリデーションは作成経路（`addMatterInfo`）でのみ実行される。
...
| 5   | ステータス遷移 + バリデーション `editMatterInfo`                                                     | `app/utils/supabase/editMatterInfo.ts` / `app/utils/matterValidation.ts`                           | `isNewApplication`（下書き→申請）/ `isPostSubmissionUpdate`（申請後更新で `has_updates`）/ 必須項目 / 請求日 > 振込期限の検出 / `getMatterValidationMessage`                                                                      | 非常に高 | バリデーションと文言は抽出済。確認文は `getUpdateMatterConfirmMessage`（`tests/utils/matterConfirmMessages.test.ts`） | 2        |
...
3. ~~**案件編集ロジックの UI 副作用分離**（#5, #8 の前提）~~: バリデーションは `matterValidation.ts`。`alert` は 6-a、`confirm` は 6-b で呼び出し側へ移動済み。現行 UI は `updateMatter` を直接呼ぶため編集時バリデーションは未実行。
