...
1. ~~**ブラウザ API との密結合**~~: `alert` は Phase 6-a、`window.confirm` は Phase 6-b で排除済み。確認は `confirmAction()`（`@mantine/modals`）。案件の必須・日付バリデーションは作成（`addMatterInfo`）と更新（`useUpdateMatter`）の両方で実行する。更新時は `start_date` を必須にしない（DB 上 nullable な既存データを他項目だけ編集して保存できるようにするため）。
...
| 5   | ステータス遷移 + バリデーション `editMatterInfo` ✅実装済（`tests/utils/matterValidation.test.ts`）  | `app/utils/supabase/editMatterInfo.ts` / `app/utils/matterValidation.ts`                                    | `isNewApplication`（下書き→申請）/ `isPostSubmissionUpdate`（申請後更新で `has_updates`）/ 必須項目 / 作成時のみ `start_date` 必須 / 請求日 > 振込期限の検出 / `getMatterValidationMessage`                                       | 非常に高 | バリデーションと文言は抽出済。確認文は `getUpdateMatterConfirmMessage`（`tests/utils/matterConfirmMessages.test.ts`） | 2        |
...
| 16  | 確認ダイアログ `confirmAction` ✅実装済（`tests/utils/confirmAction.test.tsx`）                      | `app/utils/confirmAction.tsx`                                                                               | OK で true / キャンセルで false / Esc 相当の `onClose` で false（ハングしない）/ onConfirm 後の onClose で true を維持                                                                                                            | 高       | `@mantine/modals` をモック                                                                                            | 6        |
...
3. ~~**案件編集ロジックの UI 副作用分離**（#5, #8 の前提）~~: バリデーションは `matterValidation.ts`。`alert` は 6-a、`confirm` は 6-b で呼び出し側へ移動済み。作成・更新の両方で実行する。更新時の `start_date` は必須にしない（既存 NULL データを他項目だけ編集して保存できる）。
