...
1. ~~**ブラウザ API との密結合**~~: `alert` は Phase 6-a、`window.confirm` は Phase 6-b で排除済み。確認は `confirmAction()`（`@mantine/modals`）。
...
3. ~~**案件編集ロジックの UI 副作用分離**（#5, #8 の前提）~~: バリデーションは `matterValidation.ts`。`alert` は 6-a、`confirm` は 6-b で呼び出し側へ移動済み。
