...
1. **ブラウザ API との密結合**: `editMatterInfo.ts` 等の `alert` は Phase 6-a で排除済み。`window.confirm` はヘルパ内に残っており、呼び出し側への移動は Phase 6-b。
...
3. **案件編集ロジックの UI 副作用分離**（#5, #8 の前提）: バリデーションは `matterValidation.ts` へ抽出済。`alert` は Phase 6-a で排除。`confirm` のコンポーネント側移動は Phase 6-b。
