...
未申告チームへの Slack リマインドは [4.20.6](#4206-未申告-slack-リマインド) を参照。
...
- 「前月の明細をコピー」ボタン（新規作成時のみ表示。編集時は対象外）で、対象月の前月・同チームの申告明細（種別・分類・内容・金額・担当者）を現在の明細行の末尾にまとめて取り込める（コメントは対象外。前月固有の内容の可能性が高いため）
