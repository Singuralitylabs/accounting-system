...
- PostgREST の `PGRST303`（`JWT issued at future`）は、アプリが JWT を発行しているためではなく、GoTrue が付けた `iat` を PostgREST が別ホストの時計で検証したときに起きる。`getUser()` は通るが直後の REST（選択肢マスタなど）が 401 になる、という形で観測される。クライアント側では `app/utils/supabase/postgrestFetch.ts` が **同じトークン** を短い待ちのあと再送する（refresh すると新しい `iat` で再現するため）。再試行に使う待ち時間は**合計 4 秒**（`MAX_IAT_WAIT_MS`）が上限で、`iat` のずれがこれを超える場合は待っても追い越せないため再試行せずに 401 を返す。middleware では `createTimeoutFetch`（1 リクエスト 5 秒）と合成しており、`profiles` 取得の外側の打ち切りを超えてブロックすることはない。時計ずれが 30 秒超で継続する場合は Supabase 側の NTP / サポート対応が必要。
